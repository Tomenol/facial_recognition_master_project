...
This script implements the PCA face recognition algorithm.
"""
...
def main():
	...
	# Create full, known and unknown databases
	database_known = database_full.read_known()
	database_unknown = database_full.read_unknown()

	# compute database PCs
	database_known.compute_pca(threshold=info_threshold, plot=plotting)
	database_unknown.project_pca(database_known)
...
def pixel_search_algorithm(database_tests, database_ref, threshold=0.5, threshold_norm=1916, plot=True):
	...
	for i in range(n_tested_persons):
		...
		found_id, cost_function = search(database_tests.pca[i], database_ref.pca, plot=plot)
